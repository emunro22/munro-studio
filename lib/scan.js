...
export async function scanSite(rawUrl) {
  ...
  try {
    ...
    const imageSrcs = imageMatches
      ...
      .filter(Boolean);
    const modernFormatImages = imageSrcs.filter((src) => /\.(webp|avif)(\?|$)/i.test(src)).length;
    const genericNameImages = imageSrcs.filter((src) => {
      const filename = src.split("/").pop().split("?")[0];
      return /^(img|dsc|image|photo|screenshot)[-_]?\d+\.[a-z]+$/i.test(filename);
    }).length;
    const hasModernImageFormat = imageSrcs.length > 0 && modernFormatImages > 0;
...
export function buildAutoInsights(scan, previousScan, client = null) {
    ...
    if (tradeWords.length > 0) {
      ...
      if (!tradeWords.some((w) => titleLower.includes(w))) {
        add(
          ...
          `Title is "${scan.title || "(none)"}" — it doesn't contain a word from this client's main service ("${client.trade}"). The title tag is one of the strongest ranking signals; it should lead with what the business actually does.`
        );
      }
  ...
  if (scan.imageCount > 0 && !scan.hasModernImageFormat) {
    add(
      "technical",
      "low",
      "No modern image formats (WebP/AVIF) in use",
      "All images appear to be served as jpg/png. WebP/AVIF are typically 25-50% smaller at the same visual quality — an easy win for page speed."
    );
  }
