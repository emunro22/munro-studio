...
async function scanOneCompetitor(competitor) {
  ...
  await sql`
    INSERT INTO competitor_scans (
      ...
      has_ecommerce_signals, has_video_content, has_lazy_video, has_modern_image_format,
      ...
      ${scan.hasPrivacyPolicy}, ${scan.hasTermsPage}, ${scan.hasClearCta}, ${scan.hasAnalytics},
      ${scan.hasFavicon}, ${scan.hasCookieConsent}, ${scan.hasHtmlLang}, ${scan.hasContactForm},
      ${scan.hasAboutPage}, ${scan.hasProductSchema}, ${scan.hasOrganizationSchema}, ${scan.hasEcommerceSignals},
      ${scan.hasVideoContent}, ${scan.hasLazyVideo}, ${scan.hasModernImageFormat},
...
export function buildCompetitorInsights(client, clientScan, clientReview, competitorScans) {
  ...
  if (avgReviewCount != null && clientReviewCount != null && clientReviewCount < avgReviewCount * 0.6) {
    add(
      ...
      `Nearby competitors average ~${Math.round(avgReviewCount)} reviews; this client has ${clientReviewCount}. Review count is one of the strongest local-pack ranking factors — push review requests harder.`
    );
  }
