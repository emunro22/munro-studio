...
async function fetchViaPlaces() {
  ...
  // Union into site_reviews rather than replacing — see lib/siteReviews.js.
  const merge = await mergeReviews(reviews, "places");

  return { reviews, rating: data.rating, reviewCount: data.reviewCount, source: "places", ...merge };
}
...
export async function fetchAndStoreOwnReviews() {
  ...
  if (hasOauth) {
    ...
    const merge = await mergeReviews(result.reviews, "business-profile");
    return { ...result, source: "business-profile", ...merge };
  }
